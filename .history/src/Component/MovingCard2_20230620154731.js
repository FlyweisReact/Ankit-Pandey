...
const MovingCard2 = () => {
  ...
  const [gridContHeight, setGridContHeight] = useState(0);

  useEffect(() => {
    const gridCont = document.querySelector(".gridCont");
    if (gridCont) {
      const { height } = gridCont.getBoundingClientRect();
      setGridContHeight(height);
    }
  }, []);
      ...
      <div>
        <Draggable
          ...
          bounds={{
            ...
            top: -gridContHeight + cardHeight, 
            right: homeSDimensions.width - secondCardWidth,
            bottom: gridContHeight,
          }}
        >
