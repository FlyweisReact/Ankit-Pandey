...
const Navbar = ({ width, setWidth }) => {
  ...
  const filterAppendix  = Appendix.filter((i) => i.name.toLowerCase().includes(appConst.toLowerCase()))

  console.log(localStorage.getItem('Exchange'))
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              {filterAppendix?.map((i) => (
                <Dropdown.Item onClick={() => setAppConst(i.name)}>
                  {" "}
                  {i.name}{" "}
                </Dropdown.Item>
              ))}
