...
import { Alert, Button, Container, Dropdown, Form, Modal } from "react-bootstrap";
...
const Navbar = ({ width, setWidth }) => {
      ...
      >
        <Modal.Body>
          <div className="headingClose">
            <p>Login</p>
            <i class="fa-solid fa-x" onClick={() => setModalShow(false)}></i>
          </div>
  ...
  const filterData = data.filter(
    (i) => i?.name?.toLowerCase().includes(searchConst?.toLowerCase())
  )
    ...
    <>
      ...
      <div className={darkTheme ? "darkNavbar" : ""}>
        <div className="false-navbar">
          ...
          <Dropdown>
      <Dropdown.Toggle style={{background : 'transparent' , color : '#000'}} id="dropdown-basic">
      <input
              type="search"
              placeholder="Search"
              className="Search"
      ...
      <Dropdown.Menu>
      {filterData?.map((i) => (
        <Dropdown.Item onClick={() => setSearchConst(i.name)} > {i.name} </Dropdown.Item>
      ))}
      </Dropdown.Menu>
    </Dropdown>
