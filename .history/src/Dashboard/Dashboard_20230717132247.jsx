...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (chartInstance && seriesInstance) {
      const newSeriesInstance = createSeries(chartType);
      const data = seriesInstance.data().map((point) => ({
        time: point.time,
        open: point.open,
        high: point.high,
        low: point.low,
      ...
      newSeriesInstance.setData(data);
      chartInstance.removeSeries(seriesInstance);
      chartInstance.addSeries(newSeriesInstance);
      seriesInstance = newSeriesInstance;
    }
  }, [chartType]);
