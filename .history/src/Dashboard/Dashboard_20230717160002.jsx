...
const Dashboard = () => {
  ...
  const createSeries = (chartInstance, chartType) => {
    switch (chartType) {
      case "candles":
  ...
  useEffect(() => {
    if (chartContainerRef.current) {
      ...
      seriesRef.current = createSeries(chartInstance, chartType);
  ...
  }, [chartType, chartData, createSeries]);

  useEffect(() => {
    if (chartInstance && seriesRef.current) {
      seriesRef.current.setData(chartData);
      chartInstance.timeScale().fitContent();
    }
  }, [chartData]);
