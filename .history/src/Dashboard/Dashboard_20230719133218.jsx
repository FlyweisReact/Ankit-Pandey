      ...
      if (chartType === "candles") {
        ...
        chartInstance.subscribeCrosshairMove((param) => {
          if (
            ...
            let tooltipContent = `<div style="color: rgba(0, 150, 136, 1)">Apple Inc.</div>`;
        
            // Loop through each series to get prices for the hovered time
            chartInstance
              .getVisibleSeries()
              .forEach((series) => {
                if (!series.data || series.data.length === 0) return; // Skip if no data
        
                const price = series.priceToCoordinate(param.point.y).toFixed(
                  series.priceScale().minMove.toString().length - 1
                );
        
                tooltipContent += `
                  <div style="font-size: 24px; margin: 4px 0px; color: black">
                    ${series.options.title.text}: ${price}
                  </div>
                `;
              });
        
            tooltipContent += `<div style="color: black">${dateStr}</div>`;
