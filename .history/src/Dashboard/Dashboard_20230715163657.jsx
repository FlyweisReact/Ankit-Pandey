...
const Dashboard = () => {
  ...
  const [prevDate, setPrevDate] = useState(calculatePrevDate());
  ...
  function DateGetter(date, state) {
  ...
  useEffect(() => {
    DateGetter(currentDate, setCurrentDate);
    DateGetter(prevDate, setPrevDate);
  }, []);
  ...
  const GetHistoricalDefault = async () => {
    const payload = { token, resolution: "D", Exchange , from : currentDate , to : prevDate };
    try {
