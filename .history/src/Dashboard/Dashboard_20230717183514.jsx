  ...
  useEffect(() => {
    if (chartContainerRef.current) {
      ...
      const colorfulTheme = {
        chart: {
          layout: {
            background: { type: 'solid', color: '#2e1c8c' },
            lineColor: '#fcec60',
            textColor: '#fcec60',
          },
          watermark: { color: 'rgba(0, 0, 0, 0)' },
          crosshair: { color: '#fcec60' },
          grid: {
            vertLines: { color: '#2e1c8c' },
            horzLines: { color: '#2e1c8c' },
          },
        },
        series: {
          topColor: '#fcec60',
      ...
      let chartInstance = createChart(chartContainerRef.current, chartOptions);
      ...
      chartInstance.subscribeCrosshairMove((param) => {
        if (
          ...
          const data = param.seriesPrices.get(chartInstance.series()[0]);
          const price = data !== undefined ? data : null;
          toolTip.innerHTML = `<div style="color: #2962FF">Apple Inc.</div><div style="font-size: 24px; margin: 4px 0px; color: black">${
            price ? Math.round(100 * price) / 100 : '-'
          }</div><div style="color: black">${dateStr}</div>`;

          const coordinate = chartInstance.priceToCoordinate(price);
