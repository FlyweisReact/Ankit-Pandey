  ...
  useEffect(() => {
    ...
    if (chartContainerRef.current) {
      ...
      if (chartType === "candles") {
        ...
        const toolTipWidth = 200; // Adjust the width as needed
        const toolTipHeight = 80;
        const toolTipMargin = 15;
        ...
        chartInstance.subscribeCrosshairMove((param) => {
          if (
            ...
            const dateStr = param.time;
            tooltipRef.current.style.display = "block";
            const dataPoint = mappedData.find((data) => data.time === param.time);
            if (dataPoint) {
              const price = dataPoint.value;
              ...
              const timeScaleWidth = chartInstance.timeScale().width();
              const priceScaleWidth = chartInstance.priceScale('left').width();
              const halfTooltipWidth = toolTipWidth / 2;
              left += priceScaleWidth - halfTooltipWidth;
              left = Math.min(left, priceScaleWidth + timeScaleWidth - toolTipWidth);
              left = Math.max(left, priceScaleWidth);
        
              const top = container.clientHeight / 2 - toolTipHeight / 2; // Center vertically
              tooltipRef.current.style.left = left + 'px';
              tooltipRef.current.style.top = top + 'px';
            }
          }
