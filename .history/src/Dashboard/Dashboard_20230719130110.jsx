  ...
  useEffect(() => {
    ...
    if (chartContainerRef.current) {
      ...
      if (chartType === "candles") {
        ...
        const uniqueTimestamps = new Set(); // Keep track of unique timestamps
        const mappedData = historicalData.result
          ...
          .filter((entry) => entry !== null); // Filter out null entries (duplicates)
  
        mappedData.sort((a, b) => a.time - b.time);
  
        console.log("Sorted Data:", mappedData);
        ...
        chartInstance.subscribeCrosshairMove((param) => {
          if (
            ...
            const data = param.areaSeries.get(areaSeries);
            ...
            shiftedCoordinate = Math.max(
              0,
              Math.min(container.clientWidth - 96, shiftedCoordinate)
            );
            const coordinateY =
              coordinate - 80 - 15 > 0
                ? coordinate - 80 - 15
                : Math.max(
                    0,
                    Math.min(container.clientHeight - 80 - 15, coordinate + 15)
                  );
